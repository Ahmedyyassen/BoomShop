...
const SearchBox = () => {
  ...
  return (
    <form onSubmit={handleSumbit} className={`w-fit relative flex items-center rounded-4xl border-1 ${value.length>0? "rounded-b-none":""} border-main-main`}>
    <input value={value} onChange={(e)=>setValue(e.target.value)}  type="text" placeholder="Search For Products" className="outline-0 h-12 w-[500px] py-1 px-4 rounded-l-4xl " />
    <button type="submit" className={`h-12 w-[100px] rounded-r-4xl bg-main-main text-xl flex justify-center items-center cursor-pointer`}><FaSearch className="fill-white" /></button>
